...
validate_bundle() {
  ...
  local plugin_type
  plugin_type=$(readelf -h "${bundle_dir}/lib/criu/cuda_plugin.so" 2>/dev/null | grep "Type:" | awk '{print $2}')
  if [[ "$plugin_type" != "DYN" ]]; then
    if [[ $type_errors -eq 0 ]]; then
      echo -e "${RED}FAILED${NC}"
    fi
    echo -e "  ${RED}✗${NC} lib/criu/cuda_plugin.so: expected DYN, got ${plugin_type}"
    type_errors=1
    failed=1
  fi
...
# Create tarball
if ! tar -czf "${TARBALL}" -C "${OUTPUT_DIR}" .; then
